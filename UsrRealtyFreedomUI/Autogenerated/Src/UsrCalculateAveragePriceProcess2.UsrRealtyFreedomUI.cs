namespace Terrasoft.Core.Process
{
	...
	#region Class: UsrCalculateAveragePriceProcess2MethodsWrapper

	/// <exclude/>
	public class UsrCalculateAveragePriceProcess2MethodsWrapper : ProcessModel
	{

		public UsrCalculateAveragePriceProcess2MethodsWrapper(Process process)
			: base(process) {
		...
		private bool ScriptTask1Execute(ProcessExecutingContext context) {
			...
			var typeFilter = esq.CreateFilterWithParameters(FilterComparisonType.Equal, "UsrColumn4", typeId);
			esq.Filters.Add(typeFilter);
			
			var offerTypeFilter = esq.CreateFilterWithParameters(FilterComparisonType.Equal, "UsrColumn5", offerTypeId);
